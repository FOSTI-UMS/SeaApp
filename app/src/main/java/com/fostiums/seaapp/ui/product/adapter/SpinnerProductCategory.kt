...
class SpinnerProductCategory(val context: Context, var dataSource: List<SpinnerProductModel>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        val vh: ItemHolder
        if (convertView == null) {
            view = inflater.inflate(R.layout.spinner_item_category, parent, false)
            vh = ItemHolder(view)
            view?.tag = vh
        } else {
            view = convertView
        ...
        vh.label.text = dataSource.get(position).category
...
//        vh.img.setBackgroundResource(id)
    ...
    private class ItemHolder(row: View?) {
        ...
        init {
            label = row?.findViewById(R.id.tvCategory) as TextView
        }
    }
